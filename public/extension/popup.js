...
var SUPABASE_URL = "https://oqhfmesyetdpysoyvipy.supabase.co";
var SUPABASE_ANON_KEY = "sb_publishable_75kbKg-x4w8PlMtd-sFa8g_eVXUcx7f";
...
var WEB_APP_URL = "https://ideasvault-1-0.vercel.app";
...
var DEFAULT_COLLECTIONS = [
  { id: "1", name: "\u{1F4F9} YouTube" },
  { id: "2", name: "\u{1F4F8} Instagram" },
  { id: "3", name: "\u{1F4CC} Pinterest" },
  { id: "4", name: "\u{1F426} X (Twitter)" },
  { id: "5", name: "\u{1F916} Reddit" },
  { id: "6", name: "\u{1F4BC} LinkedIn" },
  { id: "7", name: "\u{1F3A8} UI Inspiration" },
  { id: "8", name: "\u{1F4A1} Random Ideas" }
];
var state = {
  view: "loading",
  // 'login' | 'save' | 'success'
  ...
  session: null,
  collections: [],
  selectedCollection: "\u{1F4A1} Random Ideas",
  platform: "OTHER",
  url: "",
  title: "",
  description: "",
...
function suggestCollection(urlStr) {
  ...
  if (lower.includes("youtube.com") || lower.includes("youtu.be")) return "\u{1F4F9} YouTube";
  if (lower.includes("instagram.com")) return "\u{1F4F8} Instagram";
  if (lower.includes("pinterest.com") || lower.includes("pin.it")) return "\u{1F4CC} Pinterest";
  if (lower.includes("x.com") || lower.includes("twitter.com")) return "\u{1F426} X (Twitter)";
  if (lower.includes("reddit.com")) return "\u{1F916} Reddit";
  if (lower.includes("linkedin.com")) return "\u{1F4BC} LinkedIn";
  if (lower.includes("github.com")) return "\u{1F419} GitHub";
  return "\u{1F4A1} Random Ideas";
}
...
async function loadCollections() {
  try {
    ...
    if (!userId) return;
    const { data: cols, error } = await supabase.from("collections").select("*").eq("user_id", userId);
    if (error) {
...
async function uploadAndTranscribe(audioBlob, mimeType) {
  ...
  try {
    ...
    render();
    const targetApiUrl = typeof window !== "undefined" && window.location.origin.startsWith("http") && !window.location.origin.includes("chrome-extension") ? "/api/transcribe" : `${WEB_APP_URL}/api/transcribe`;
...
async function handleSaveInspiration() {
  ...
  try {
    ...
    const boardName = state.selectedCollection || "\u{1F4A1} Random Ideas";
    let collectionId = null;
    if (Array.isArray(state.collections)) {
      const found = state.collections.find(
        (c) => c.name === boardName || c.name.replace(/[^\w\s]/g, "").trim().toLowerCase() === boardName.replace(/[^\w\s]/g, "").trim().toLowerCase()
      );
    ...
    if (!collectionId) {
      const { data: existingColls } = await supabase.from("collections").select("*").eq("user_id", userId);
      const matchingColl = (existingColls || []).find(
        (c) => c.name === boardName || c.name.replace(/[^\w\s]/g, "").trim().toLowerCase() === boardName.replace(/[^\w\s]/g, "").trim().toLowerCase()
      );
      if (matchingColl) {
        ...
        const icon = boardName.split(" ")[0] || "\u{1F4A1}";
        const { data: newCol, error: newColErr } = await supabase.from("collections").insert({
          id: newCollId,
          name: boardName,
          user_id: userId,
          icon
    ...
    const { error: saveErr } = await supabase.from("ideas").insert(dbPayload);
    if (saveErr) {
...
function render() {
    ...
    container.innerHTML = `
      <div style="padding: 24px; display: flex; flex-direction: column; gap: 20px;">
        <div style="display: flex; align-items: center; gap: 10px;">
          <div style="width: 32px; height: 32px; background: linear-gradient(135deg, #4F8CFF, #8B5CF6); border-radius: 8px; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 16px;">\u{1F4A1}</div>
          <div>
  ...
  let voiceBtnLabel = "\u{1F399}\uFE0F Record Voice";
  ...
  if (state.micPermissionDenied) {
    ...
    voiceBtnLabel = `\u{1F534} Recording... (${formatTime(state.recordSeconds)})`;
    voiceBtnBg = "#EF4444";
